...
import { assistantApi, roadmapsApi } from "@/lib/api/client";
...
function asViews(raw: any[]): RoadmapView[] {
  return (raw || []).map((r) => ({
    id: String(r.id),
    title: r.title,
    templateKey: r.templateKey || r.template_key || null,
...
export default function AssistantChat({ embedded = false }: { embedded?: boolean }) {
  ...
  const quick = [t("quickVisa"), t("quickMove"), t("quickToday"), t("quickMentor")];
  ...
  const loadRoadmaps = useCallback(async () => {
    ...
    const res = await roadmapsApi.list("active");
    setRoadmaps(asViews((res.data?.roadmaps || []) as any[]));
  }, [isAuthenticated]);
  ...
  const runAction = async (action: any) => {
    ...
    if (action.type === "create_roadmap") {
      if (!isAuthenticated) {
        setMessages((m) => [
          ...m,
          { id: `${Date.now()}-login`, role: "assistant", content: t("needLogin") },
        ]);
        return;
      }
      setLoading(true);
  ...
  const toggleStep = async (roadmapId: string, stepId: string, next: boolean) => {
    setRoadmaps((prev) =>
      prev.map((r) => {
        if (r.id !== roadmapId) return r;
        const steps = r.steps.map((s) => (s.id === stepId ? { ...s, completed: next } : s));
        const done = steps.filter((s) => s.completed).length;
        return { ...r, steps, progress: Math.round((done / Math.max(steps.length, 1)) * 100) };
      })
    );
    ...
    <div className="space-y-4">
      {isAuthenticated ? (
        <div className="rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
          <h2 className="text-sm font-bold text-zinc-900 dark:text-white mb-3">{tRoad("myRoadmaps")}</h2>
          <RoadmapChecklist
            roadmaps={roadmaps}
            onToggle={(id, stepId, next) => void toggleStep(id, stepId, next)}
      ...
      <div
        className={`flex flex-col ${
          embedded ? "h-[calc(100vh-22rem)] min-h-[22rem]" : "min-h-[50vh]"
