...
export default function StudyInfoCard({ info }: StudyInfoCardProps) {
  ...
  return (
    <article
      id={`article-${info.id}`}
      className="group bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-700 p-6 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 relative overflow-hidden scroll-mt-28"
    >
      {/* Gradient accent bar */}
      <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${gradientClass}`}></div>
      
      <div className="relative">
        <h3 className="text-xl font-bold text-gray-900 dark:text-slate-100 mb-3 group-hover:text-primary-600 transition-colors">
          {info.title}
        </h3>
        <p className="text-gray-600 dark:text-slate-400 mb-5 line-clamp-6 leading-relaxed whitespace-pre-line">
          {info.content}
        </p>
