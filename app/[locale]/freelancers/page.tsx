...
export default async function FreelancersPage({
  ...
  let groups: FreelancerGroup[] = [];
  try {
    groups = await queryFreelancerGroups({ limit: 200 });
  } catch {
    groups = [];
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50">
      <div className="relative overflow-hidden bg-gradient-to-br from-orange-600 via-red-500 to-pink-500 py-16">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-14">
        {groups.length === 0 ? (
          <EmptyState
            title="프리랜서 그룹이 없습니다"
            description="시드 스크립트로 샘플 그룹을 추가할 수 있습니다."
            actionLabel="홈으로"
            actionHref={`/${locale}`}
          />
        ) : (
